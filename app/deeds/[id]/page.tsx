...
async function getDeed(id: string): Promise<DeedWithRelations | null> {
  ...
  // Get the deed
  const { data: deed, error: deedError } = await supabase.from("deeds").select("*").eq("id", id).single()
  ...
  // Get the checks
  const { data: checks, error: checksError } = await supabase.from("checks").select("*").eq("deed_id", id)
  ...
  // Get the nominees
  const { data: nominees, error: nomineesError } = await supabase.from("nominees").select("*").eq("deed_id", id)
    ...
    <main className="container mx-auto py-8 px-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 space-y-6">
          <Card>
            <CardContent className="pt-6">
              ...
              <div className="space-y-2">
                ...
                <p>
                  <span className="font-medium bangla">Full Name:</span> <span className="bangla">{deed.fullname}</span>
                </p>
                <p>
                  <span className="font-medium bangla">Father&apos;s Name:</span>{" "}
                  <span className="bangla">{deed.fathersname}</span>
                </p>
                <p>
                  <span className="font-medium bangla">Mother&apos;s Name:</span>{" "}
          ...
          <Card>
            <CardContent className="pt-6">
              <h2 className="text-xl font-semibold mb-4 bangla">Current Address</h2>
              <div className="space-y-2">
                <p>
          ...
          <Card>
            <CardContent className="pt-6">
              <h2 className="text-xl font-semibold mb-4 bangla">Permanent Address</h2>
              <div className="space-y-2">
                <p>
                  <span className="font-medium bangla">Village:</span>{" "}
                ...
                <div className="space-y-4">
                  {deed.checks.map((check) => (
                    <div key={check.id} className="p-3 border rounded-md">
                      <p>
                        <span className="font-medium">Check Number:</span> {check.check_number}
                      </p>
                      <p>
                      ...
                      <p>
                        <span className="font-medium">Amount:</span> {check.amount.toLocaleString()} BDT
                      </p>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-4">
                  {deed.nominees.map((nominee) => (
                    <div key={nominee.id} className="p-3 border rounded-md">
                      <p>
                        <span className="font-medium bangla">Name:</span> <span className="bangla">{nominee.name}</span>
                      </p>
                      <p>
                        <span className="font-medium bangla">Father&apos;s Name:</span>{" "}
                      ...
                      <p>
                        <span className="font-medium">Mobile:</span> {nominee.mobile}
                      </p>
                      <p>
                        <span className="font-medium bangla">Relationship:</span>{" "}
                        <span className="bangla">{nominee.relationship}</span>
                      </p>
                      <p>
                        <span className="font-medium">Distributed Portion:</span> {nominee.distributed_portion}%
                      </p>
                    </div>
                  ))}
                </div>
